...
import CartOrderUI from '../Orders/CartOrderUI';
...
export function CartOrder():ReactElement {
  ...
  useEffect(() => {
    ...
    (async()=>{
        if(auth===true){
        const {data}:{data:{message:string,order:{product:productType,qty:number}[]|undefined,status:boolean,error:[]|undefined|string}} =await axios.get(import.meta.env.VITE_API_BASE_URL+`/order/getSingleOrderAdmin/${id}`,
            {
...
export default adminGards(CartOrder)  as typeof CartOrder;