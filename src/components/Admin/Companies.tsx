...
export function Companies():ReactElement {
  ...
  const handleRemoveCompanies = useCallback((id:string)=>{
        Swal.fire({
          ...
          if (result.isConfirmed) {
            const {data}:{data:{message:string,status:boolean,errors:string|undefined|[]}} = await axios.delete(import.meta.env.VITE_API_BASE_URL+`/company/deleteCompany/${id}`,{
  ...
  const getCompanies = useCallback(async(name:string|null,check:boolean,page:number|null)=>{
      try{
        setCompanies(()=>null);
        const {data}:{data:{message:string,status:boolean,numberOfPage:number|undefined,companies:undefined|categoryType[],errors:string|undefined|[]}} = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/company/getCompanysPagination?page=${page}${(name)?`&name=${name}`:''}`);
        if(data.companies&&Number.isInteger(data.numberOfPage)){
            if(check){
      ...
      (async()=>{
        if(auth){
          if(name){
            handler = setTimeout(()=>{
              (async()=>{
                await getCompanies(name,check,page);
              })()
            },700)
          }
          else{
              (async()=>{
                await getCompanies(null,check,page);
              })()
          }
        }
      })()
  ...
  },[name,requestDone,page,auth,getCompanies]);
  return (
    <>
    <section className={`row w-100 m-0 flex-grow-1 align-items-start my-4 `}>
...

      {
            ...
            {(numberOfPages)&&<nav className='mt-1 d-flex justify-content-center' aria-label="Page navigation example">
          ...
          </div></>:<h1 className='text-center w-100 text-primary text-capitalize'>no companies for now</h1>):<Loading  classNamePropert=''/>
      }
    </section>
    </>
  )
...
export default adminGards(Companies) as typeof Companies;